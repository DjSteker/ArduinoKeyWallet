...
// Versión: v3.6 (Generación de contraseñas con pulsación simultánea UP+DOWN)
// =============================================

#define COMPILATION_NAME    "KeyVault"
#define COMPILATION_DATE    __DATE__
#define COMPILATION_TIME    __TIME__
#define COMPILATION_VERSION "v3.6-alt"
...
static const unsigned long T_SIMULT_PRESS  = 3000; // 3 segundos para pulsación simultánea
...
// -----------------------------------------------------------------------------
// Keyboard (sin layout de teclado)
// -----------------------------------------------------------------------------
...
void handleInput() {
  ...
  switch (menuMode) {
    ...
    case MODE_EDIT:
      // Detección de pulsación simultánea UP+DOWN para generar contraseña
      if (editField == FIELD_CONTENT && bUp.state && bDown.state) {
        unsigned long simultaneousPressTime = millis() - bUp.pressTime;
        if (simultaneousPressTime >= T_SIMULT_PRESS) {
          ...
          refreshNeeded = true;
          bUp.longFired = true; // Evitar que se active otra acción
