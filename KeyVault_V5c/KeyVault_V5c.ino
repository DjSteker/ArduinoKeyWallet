...
static const unsigned long T_VIEW_MSG = 1200000;     // 1.2s
static const unsigned long T_LONG_PRESS = 2000000;   // 2s
static const unsigned long T_TIMEOUT = 30000000;     // 30s
static const unsigned long T_DEBOUNCE = 30000;       // 30ms
static const unsigned long T_REPT_START = 400000;    // 400ms
static const unsigned long T_REPT_FAST = 40000;      // 40ms
static const unsigned long T_SIMULT_PRESS = 300000;  // 300ms
...
struct Button {
  ...
  unsigned long lastChange = 0;  // En micros()
  unsigned long pressTime = 0;   // En micros()
  unsigned long lastRepeat = 0;  // En micros()
  bool simultaneousPressStarted = false;
};
...
static char &activeChar() {
  if (editField == FIELD_TITLE) return dataset[activeRecord].header[cursorPos];
  else return dataset[activeRecord].details[cursorPos];
}
...
void generateRandomPassword(char *password, uint8_t length) {
  const char charset[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+-=[]{}|;:,.<>?";
  for (uint8_t i = 0; i < length; i++) password[i] = charset[rand() % (sizeof(charset) - 1)];
  password[length] = '\0';
}
...
static void writeStateToEEPROM() {
  for (uint8_t i = 0; i < 4; i++) EEPROM.update(EE_MAGIC0 + i, EE_MAGIC_VAL[i]);
...
static void saveRecord(uint8_t idx) {
  uint16_t addr = recordAddr(idx);
  for (uint8_t i = 0; i <= MAX_TITLE_LEN; i++) EEPROM.update(addr + i, dataset[idx].header[i]);
  uint16_t cAddr = addr + MAX_TITLE_LEN + 1;
  for (uint8_t i = 0; i <= MAX_CONTENT_LEN; i++) EEPROM.update(cAddr + i, dataset[idx].details[i]);
...
static void pollButton(Button &b, uint8_t pin) {
  ...
  if ((now - b.lastChange) >= T_DEBOUNCE && raw != b.state) {
    ...
    if (raw) {
      ...
      b.longFired = false;
      b.lastRepeat = now;
      if (!b.simultaneousPressStarted) b.simultaneousPressStarted = true;
    } else {
      b.released = true;
      b.simultaneousPressStarted = false;
    }
    refreshNeeded = true;
  }
}
...
static bool checkRepeat(Button &b) {
  ...
  uint16_t interval = (held < 2000000) ? 120000 : T_REPT_FAST;
...
static void updateDisplay() {
  if (displaySleeping || !refreshNeeded) return;
  display.clearDisplay();
  display.setCursor(0, 0);

  if (showSavedMsg) display.println(F("\n >> Guardado! <<"));
  else {
    switch (menuMode) {
      case MODE_VIEW:
        display.print(F("Reg "));
        display.print(activeRecord + 1);
        if (dataset[activeRecord].dirty) display.print('*');
        display.println();
        printWindowed(dataset[activeRecord].header, 0, false);
        display.println();
        printWindowed(dataset[activeRecord].details, 0, false);
        display.println(F("U/D:Nav ENT:Edit"));
        break;
      ...
      case MODE_EDIT:
        {
          ...
          display.println(']');
          printWindowed(dataset[activeRecord].header, cursorPos, editField == FIELD_TITLE);
          display.setCursor(0, 16);
          printWindowed(dataset[activeRecord].details, cursorPos, editField == FIELD_CONTENT);
      ...
      case MODE_CONFIG:
        ...
        display.println(F("No disponible"));
        break;
...
void tareaModoView() {
  ...
  if ((now - lastInteraction) > T_TIMEOUT) {
...
void handleInput() {
  ...
  if (bUp.pressed || bDown.pressed || bEnter.pressed) {
    switch (menuMode) {
      case MODE_VIEW: tramaDisplay.fun(tareaModoView); break;
      case MODE_EDIT: tramaDisplay.fun(tareaModoEdit); break;
      case MODE_CONFIRM: tramaDisplay.fun(tareaModoConfirm); break;
      case MODE_CONFIG: tramaDisplay.fun(tareaModoConfig); break;
      case MODE_UNSAVED: tramaDisplay.fun(tareaModoUnsaved); break;
    }
  ...
  switch (menuMode) {
    ...
    case MODE_EDIT:
      if (editField == FIELD_CONTENT && bUp.simultaneousPressStarted && bDown.simultaneousPressStarted) {
        unsigned long simultaneousPressTime = now - bUp.pressTime;
        if (simultaneousPressTime >= T_SIMULT_PRESS && !bUp.longFired && !bDown.longFired) {
          generateRandomPassword(dataset[activeRecord].details, MAX_CONTENT_LEN);
          cursorPos = strlen(dataset[activeRecord].details);
      ...
      if (bUp.pressed || checkRepeat(bUp)) {
        if (activeChar() < CHAR_MAX) activeChar()++;
        else activeChar() = CHAR_MIN;
        dataset[activeRecord].dirty = true;
      }
      if (bDown.pressed || checkRepeat(bDown)) {
        if (activeChar() > CHAR_MIN) activeChar()--;
        else activeChar() = CHAR_MAX;
        dataset[activeRecord].dirty = true;
      }
      if (bEnter.released && !bEnter.longFired) {
        if (cursorPos < activeFieldMax() - 1) cursorPos++;
        else if (editField == FIELD_TITLE) {
          editField = FIELD_CONTENT;
          cursorPos = 0;
        } else menuMode = MODE_CONFIRM;
      }
      if (bEnter.state && !bEnter.longFired && (now - bEnter.pressTime > T_LONG_PRESS)) {
        bEnter.longFired = true;
    ...
    case MODE_CONFIRM:
      ...
      if (bDown.pressed) menuMode = MODE_EDIT;
      break;

    case MODE_CONFIG:
      if (bEnter.pressed) menuMode = MODE_VIEW;
      break;
    case MODE_UNSAVED:
      if (bUp.pressed) {
        saveRecord(activeRecord);
...
void setup() {
  ...
  // Mensaje al arrancar
  Serial.println(F("---"));
  Serial.print(F(COMPILATION_NAME));
  Serial.print(F(" "));
  Serial.print(F(COMPILATION_DATE));
  Serial.print(F(" "));
  Serial.print(F(COMPILATION_TIME));
  Serial.print(F(" "));
  Serial.println(F(COMPILATION_VERSION));
  ...
  Serial.println(F("KeyVault listo para usar."));
  Serial.println(F("--------------------------------"));
}
